...
class GBRBM(object):
    ...
    def fitmodel(self,n_epoch,batch_size,eeta): # training given input x
    # n_epoch - number of epoches
    # batch_size = minibatch size
    # eeta = learning rate
        N = self.x.shape[0] # N = number of samples
        for epoch in range(n_epoch):
            for n in range(0,N-batch_size+1,batch_size):
                vo = self.x[n:n+batch_size,:]
                _,aho = self.solve_for_hl(vo) # get probabilities of hidden layer state
                
                #k step contrastive divergence, k = 1
                vk = self.x[n:n+batch_size,:]
                for k in range(1):
    ...
    def testmodel(self,z): # calculate error of the RBM model for the given input x
        N = z.shape[0] # Number of samples
        v1 = np.empty([N,self.nv])
        e = 0
        for n in range(N):
            vo = z[n,:]
            h1,_ = self.solve_for_hl(vo)  # calculate hidden layer state
            v1[n,:],_ = self.solve_for_vl(h1)  # calculate visible layer state
            e += np.mean((vo - v1[n,:])**2) # calculate error
        return e/N,v1
    
    def get_hiddenlayer(self,z): # get hidden layer states for given input x
        N = z.shape[0]
...
"Converting the input and target data corresponding to training and validation torch tensors"
